...
Extract the Windows ZIP or Linux tarball into a writable folder before running
it. On Windows, check `%LOCALAPPDATA%\HOI4MapEditor\logs`; on Linux, check
`$XDG_STATE_HOME/HOI4MapEditor/logs` or
`~/.local/state/HOI4MapEditor/logs`. A source build can be checked with
`cargo run`.
...
The editor keeps pending project changes when encoding, staging, validation,
backup, or replacement fails. Source files remain unchanged before commit; a
failure during commit triggers verified rollback. Do not delete the reported
backup if recovery is required.
...
Global settings are under `%APPDATA%\HOI4MapEditor\config.toml` on Windows and
`$XDG_CONFIG_HOME/HOI4MapEditor/config.toml` (or
`~/.config/HOI4MapEditor/config.toml`) on Linux. Project settings are under
...
the platform configuration path is writable. If the file's `language` value is
unrecognized (for example, edited by hand or written by a newer version), the
editor falls back to English for that session and logs a warning; it does not
silently overwrite your file.

## Boxes or missing characters instead of text ("tofu")

The editor bundles a single Latin font and, for Cyrillic (Русский) and
Simplified Chinese (简体中文) text it cannot cover, falls back to available
system fonts. Windows checks Segoe UI, Microsoft YaHei UI, and SimSun; Linux
checks common Noto, DejaVu, Liberation, and WenQuanYi locations. Install an
appropriate system language/font package and restart the editor if characters
still render as empty boxes. No font is silently swapped for different
characters — only the glyphs drawn to render the text you already see.
...
Each release publishes a `.sha256` file alongside its archive. On Windows,
compare it with:
...
On Linux, run `sha256sum -c HOI4-Map-Editor-v<version>-linux-x86_64.tar.gz.sha256`
