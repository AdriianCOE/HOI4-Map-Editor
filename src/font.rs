...
impl<'a> CharacterCache for MultiFontGlyphCache<'a> {
  ...
  fn character(&mut self, size: FontSize, ch: char) -> Result<Character<'_, Texture>, Self::Error> {
...
mod tests {
  ...
  fn primary_font_covers_accented_latin_directly() {
    ...
    for ch in "áàâãéêíóôõúçàâæçéèêëîïôœùûüÿáéíóúüñ".chars() {
      assert_eq!(cache.cache_index_for(ch), 0, "'{ch}' should not need a fallback font");
    }
  }
  ...
  fn cyrillic_and_cjk_resolve_to_a_fallback_font_when_one_is_available() {
    ...
    for ch in "АБВЖЗПРСабвжзпрс".chars() {
      assert_ne!(cache.cache_index_for(ch), 0, "Cyrillic '{ch}' must use a fallback font");
    }
    for ch in "设置省份状态应用更改保存地图".chars() {
      assert_ne!(cache.cache_index_for(ch), 0, "CJK '{ch}' must use a fallback font");
    }
  }
}
