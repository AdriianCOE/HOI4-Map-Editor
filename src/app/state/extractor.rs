...
impl<'a> Extractor<'a> {
  ...
  fn extract(mut self) -> ExtractStateResult {
    ...
    let state_blocks: Vec<usize> = self
      ...
      .iter()
      .enumerate()
      .filter(|(_, entry)| entry.key_text() == Some("state") && entry.value.as_block().is_some())
      .map(|(index, _)| index)
      .collect();
  ...
  fn extract_state_block(&mut self, block: &PdxBlock) -> StateData {
    ...
    for entry in &block.entries {
      match entry.key_text() {
        Some("id") => self.read_u32(entry, ProjectDiagnosticKind::InvalidStateId, |value| {
          state.id = Some(value);
        }),
        Some("name") => {
    ...
    if state.provinces.is_empty() {
      ...
      self.push(kind, DiagnosticSeverity::Error, Some(block.span), "missing provinces");
    }
  ...
  fn extract_victory_points(&mut self, entry: &PdxEntry, state: &mut StateData) {
    ...
    for pair in scalars.chunks(2) {
      ...
      state.history.victory_points.push(VictoryPoint { province_id, value });
    }
  }
...
mod tests {
  ...
  history={
    ...
    assert_eq!(state.history.state_buildings.get("infrastructure"), Some(&3));
    assert_eq!(
      state
        .history
        .province_buildings
