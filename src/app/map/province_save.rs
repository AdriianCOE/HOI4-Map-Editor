...
impl ProvinceSaveReport {
    pub fn summary_text(&self) -> String {
        ...
        let files = self
            ...
            .collect::<Vec<_>>()
            .join("\n");
        let backup = self.backup_id.as_deref().map_or_else(
            || "Backup: not applicable".to_owned(),
            |id| format!("Backup: {id}"),
        );
        format!(
...
pub(crate) fn build_province_map_candidate(
    bundle: &Bundle,
) -> Result<ProvinceMapCandidate, String> {
    build_province_map_candidate_inner(bundle, Fault::None, |_, _, _| {})
...
fn build_province_map_candidate_inner(
    ...
    let mut bmp = ProgressWriter::new(
        Vec::with_capacity(expected_bmp_size as usize),
        expected_bmp_size,
        |current, total| {
...
fn execute_province_save_with_fault(
    ...
    let (candidate, timings) =
        build_province_map_candidate_inner(bundle, fault, |stage, current, total| {
    ...
    match location {
        ...
        Location::ZipArchive(path) => {
            if mode == ProvinceSaveMode::Save {
                return Err(
                    "Saving the current Province workspace to an archive is not supported."
                        .to_owned(),
                );
            }
            execute_archive(
    ...
    let result = (|| {
        ...
        if let Err(error) = commit_result {
            ...
            return match rollback {
                Ok(()) => Err(format!(
                    "{error}. Original files were restored and verified."
                )),
        ...
        if let Some(path) = journal_path.as_deref() {
            write_journal(
                ...
                &ProvinceSaveJournal {
                    ...
                    committed_files: prepared.iter().map(|file| file.relative.clone()).collect(),
                },
            )?;
        }
        ...
        let post_commit = (|| {
            ...
            validate_province_map_candidate(&candidate, |relative| {
                ...
                fs::read(&path)
                    .map_err(|error| format!("Cannot reload {}: {error}", path.display()))
            })?;
...
fn execute_archive(
    ...
    let result = (|| {
        ...
        if names != expected {
            return Err(
                "Staged archive contains files outside the province export allowlist.".to_owned(),
            );
        }
        validate_province_map_candidate(&candidate, |relative| {
            staged_archive
        ...
        if fault == Fault::AfterCommit(1) {
            ...
            return Err(
                "Injected failure after archive commit; original archive restored.".to_owned(),
            );
        }
    ...
    files
        ...
        .map(|relative| {
            ...
            Ok(PreparedFile {
                ...
                before: baseline.get(relative).cloned().ok_or_else(|| {
                    format!("Missing destination baseline for {}", relative.display())
                })?,
                destination,
            })
        })
...
fn validate_bmp(bytes: &[u8], expected: &image::RgbImage) -> Result<(), String> {
    ...
    if decoded != *expected {
        return Err(
            "Staged provinces.bmp pixels do not match the working province model.".to_owned(),
        );
    }
    Ok(())
}
...
fn validate_definition_colors(
    ...
    if defined != pixels {
        return Err(
            "Staged BMP colors and definition.csv province colors do not match.".to_owned(),
        );
    }
    Ok(())
}
...
fn verify_backup(manifest_path: &Path) -> Result<(), String> {
    ...
    let manifest: ProvinceBackupManifest = toml::from_slice(&bytes).map_err(|error| {
        format!(
...
fn commit_files(
    ...
    for index in 0..prepared.len() {
        ...
        if fault == Fault::AfterCommit(index + 1) {
            return Err(format!(
                "Injected failure after {} committed file(s)",
                index + 1
            ));
        }
        #[cfg(test)]
...
fn rollback_files(prepared: &[PreparedFile]) -> Result<(), String> {
    ...
    for file in prepared.iter().rev() {
        let committed = file.rollback.exists()
            || (file.before.is_none() && file.destination.exists() && !file.stage.exists());
        if committed
            && let Err(error) =
                rollback_one(&file.destination, &file.rollback, file.before.is_some())
        {
            errors.push(error);
        }
    }
        ...
        match &file.before {
            Some(expected) => match fs::read(&file.destination) {
                ...
                Err(error) => errors.push(format!(
                    "Cannot verify rollback {}: {error}",
                    file.destination.display()
                )),
            },
            None if file.destination.exists() => errors.push(format!(
                "Rollback left created file {}",
                file.destination.display()
            )),
            None => {}
        }
...
fn verify_published_files(
    ...
    for (index, (relative, expected)) in candidate.iter().enumerate() {
        ...
        if actual != *expected {
            return Err(format!(
                "Final file differs from validated candidate: {}",
                path.display()
            ));
        }
...
fn create_lock(path: &Path, transaction_id: &str) -> Result<(), String> {
    ...
    let mut file = OpenOptions::new()
        ...
        .map_err(|error| {
            format!(
                "Another Province Save is active ({}): {error}",
                path.display()
            )
        })?;
...
fn internal_root(map_directory: &Path) -> PathBuf {
    ...
    {
        map_directory.parent().unwrap_or(map_directory).to_owned()
    } else {
...
fn check_cancel(cancellation: &ProvinceSaveCancellation) -> Result<(), String> {
    if cancellation.cancelled() {
        Err(
            "Province map save cancelled before commit; destination files were not changed."
                .to_owned(),
        )
    } else {
        Ok(())
...
mod tests {
    ...
    fn bmp_validation_requires_24_bit_padded_complete_output() {
        let bundle = bundle();
        let mut bytes = Vec::new();
        super::super::bridge::write_rgb_bmp_image(&mut bytes, &bundle.map.base.color_buffer)
            .unwrap();
    ...
    fn directory_save_publishes_complete_candidates_and_writes_verified_backup() {
        ...
        assert_eq!(
            fs::read(
                root.join(".hoi4-state-editor/backups")
                    .join(report.backup_id.unwrap())
                    .join("province-map/files/provinces.bmp")
            )
            .unwrap(),
            b"old-bmp"
        );
        assert!(
            read_rgb_bmp_image(fs::read(map.join("provinces.bmp")).unwrap().as_slice()).is_ok()
        );
    ...
    fn failures_across_the_save_pipeline_preserve_originals_and_clean_staging() {
        ...
        for (index, fault) in faults.into_iter().enumerate() {
            ...
            assert!(!root.join(".hoi4-state-editor/province-save.lock").exists());
            fs::remove_dir_all(root).unwrap();
        }
    }
    ...
    fn incomplete_rollback_keeps_lock_and_marks_recovery_required() {
        ...
        assert!(root.join(".hoi4-state-editor/province-save.lock").is_file());
        let journal: ProvinceSaveJournal = toml::from_slice(
            &fs::read(root.join(".hoi4-state-editor/province-save-journal.toml")).unwrap(),
        )
        .unwrap();
    ...
    fn external_change_cancels_without_overwriting_the_external_bytes() {
        ...
        assert_eq!(fs::read(map.join("provinces.bmp")).unwrap(), b"old-bmp");
        assert_eq!(
            fs::read(map.join("definition.csv")).unwrap(),
            b"external-change"
        );
        fs::remove_dir_all(root).unwrap();
    }
    ...
    fn real_mod_province_atomic_save_smoke_on_controlled_copy() {
        ...
        for name in ["provinces.bmp", "definition.csv"] {
            assert_eq!(
                fs::read(source_map.join(name)).unwrap(),
                original_bytes[name]
            );
