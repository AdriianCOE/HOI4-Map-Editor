...
use super::{Bundle, Color, Extents, Map, boundary_to_line};
use crate::app::canvas::CameraCombo;
use crate::app::colors;
use crate::util::{XYIter, stringify_color};
            ...
            Problem::FewSharedBorders(_, ref borders) if extras => {
                if camera_combo.camera.scale_factor() > 1.0 {
                    ...
                    for &boundary in borders.iter() {
                        let [b1, b2] = boundary_to_line(boundary).into_array().map(vec2_u32_to_f64);
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            Problem::TooLargeBox(extents) => {
                write!(
                    f,
                    "Province has too large box from {:?} to {:?}",
                    extents.upper, extents.lower
                )
            }
            Problem::TooFewPixels(count, [x, y]) => {
                write!(
                    f,
                    "Province has only {} pixels around [{:.0}, {:.0}]",
                    count, x, y
                )
            }
            ...
            Problem::FewSharedBorders(boundary, ref borders) => {
                ...
                write!(
                    f,
                    "Only {} shared borders between provinces {} and {}",
                    borders.len(),
                    a,
                    b
                )
...
fn draw_dot(
    ...
    let [x, y] = camera_combo.compute_position(pos);
    Ellipse::new(color).draw_from_to(
        [x - 4.0, y - 4.0],
        [x + 4.0, y + 4.0],
        &Default::default(),
        ctx.transform,
...
fn draw_box(
    ...
    Rectangle::new_border(color, 1.0).draw_from_to(
        lower,
        upper,
        &Default::default(),
        ctx.transform,
...
pub fn analyze(bundle: &Bundle) -> Vec<Problem> {
    ...
    let mut province_extents =
        AHashMap::<Color, Extents>::with_capacity(bundle.map.provinces_count());
    ...
    for pos in XYIter::new(0..width, 0..height) {
        ...
        if extras && bundle.config.extra_warnings.few_shared_borders {
            if pos[0] + 1 < width {
                ...
                let borders = borders.entry(UOrd::new([a, b])).or_insert_with(Vec::new);
    ...
    for (color, extents) in province_extents {
        ...
        if province_data.pixel_count <= 8 {
            ...
            problems.push(Problem::TooFewPixels(
                province_data.pixel_count,
                center_of_mass,
            ));
        };
...
fn is_crossing_at(map: &Map, [x0, y0]: Vector2<u32>) -> bool {
    ...
    let [x1, y1] = [if x0 + 1 == map.width() { 0 } else { x0 + 1 }, y0 + 1];
