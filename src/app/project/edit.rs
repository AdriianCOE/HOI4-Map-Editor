...
impl StateEditSession {
  pub fn new(project: &Hoi4Project, map: &Map) -> Self {
    ...
    let known_state_ids = project.states.iter()
      .filter_map(|document| document.data.as_ref())
      .filter_map(|data| data.id)
      .filter(|&id| id != 0)
      .chain(project.states.iter().filter_map(|document| state_id_from_filename(&document.path)))
      .collect::<BTreeSet<_>>();
    let mut state_id_counts = BTreeMap::<u32, usize>::new();
    for state_id in project.states.iter()
      .filter_map(|document| document.data.as_ref())
      .filter_map(|data| data.id)
    {
      *state_id_counts.entry(state_id).or_default() += 1;
    }
    let valid_state_ids = project.states_by_id.iter()
  ...
  pub fn state_province_count(&self, state_id: u32) -> usize {
    self.working.provinces_by_state.get(&state_id).map_or(0, BTreeSet::len)
  }
  ...
  pub fn validate_state_creation(
    ...
    self.build_create_command(state_id, properties.clone(), &province_ids).map(drop)
  }
  ...
  pub fn selection_sources(&self) -> BTreeMap<Option<u32>, usize> {
    ...
    for province_id in &self.selected_provinces {
      *sources.entry(self.working.state_by_province.get(province_id).copied()).or_default() += 1;
    }
    sources
  }
  ...
  pub fn province_data(&self, province_id: u32) -> Option<EditableProvinceData> {
    self.province_kinds.contains_key(&province_id)
      .then(|| self.working.province_data(self.province_state_id(province_id), province_id))
  }
  ...
  pub fn editable_province_state(&self, province_id: u32) -> Result<u32, StateEditError> {
    ...
    self.validate_target_state(state_id)?;
    self.working.properties_by_state.get(&state_id)
  ...
  pub fn summary(&self) -> StateEditSummary {
    StateEditSummary {
      ...
      session_errors: self.session_diagnostics.iter()
        .filter(|diagnostic| diagnostic.severity == DiagnosticSeverity::Error)
        .count(),
      session_warnings: self.session_diagnostics.iter()
        .filter(|diagnostic| diagnostic.severity == DiagnosticSeverity::Warning)
        .count(),
      active_states: self.valid_state_ids.len(),
      created_states: self.valid_state_ids.iter()
        .filter(|state_id| self.origins_by_state.get(state_id) == Some(&WorkingStateOrigin::CreatedInSession))
        .count(),
  ...
  pub fn apply_lasso_selection(
    ...
    match mode {
      ...
      LassoSelectionMode::Remove => {
        self.selected_provinces.retain(|province_id| !province_ids.contains(province_id));
      },
    }
  ...
  pub fn select_target_state_provinces(&mut self) -> Result<usize, StateEditError> {
    ...
    self.selected_provinces = self.working.provinces_by_state
      .get(&target_state_id)
      .into_iter()
      .flatten()
      .copied()
  ...
  pub fn reassign_selected(&mut self, target_state_id: Option<u32>) -> Result<(), StateEditError> {
  ...
  pub fn update_state_properties(
    ...
    self.validate_target_state(state_id)?;
    self.last_changed_provinces.clear();
    let before = self.working.properties_by_state.get(&state_id)
      .cloned()
      .ok_or(StateEditError::TargetStateNotFound(state_id))?;
    if before == after {
      return Ok(false);
  ...
  fn build_create_command(
    ...
    properties.validate().map_err(StateEditError::InvalidStateProperties)?;
  ...
  pub fn update_province_data(
    ...
    self.last_changed_provinces.clear();
    after.validate().map_err(StateEditError::InvalidProvinceData)?;
  ...
  pub fn undo(&mut self) -> bool {
    ...
    self.last_changed_provinces.extend(command.changed_provinces());
    match &command {
      StateEditCommand::CreateState { state, .. } if self.target_state_id == Some(state.state_id) => {
        self.target_state_id = None;
      },
  ...
  pub fn discard(&mut self) {
    let started = Instant::now();
    self.last_changed_provinces = self.baseline.state_by_province.keys()
    ...
    self.origins_by_state.retain(|_, origin| matches!(origin, WorkingStateOrigin::Loaded { .. }));
  ...
  pub fn validate_invariants(&self) -> Result<(), StateEditError> {
    for (province_id, state_id) in &self.working.state_by_province {
      if !self.working.provinces_by_state
        .get(state_id)
  ...
  fn build_province_deltas(
    ...
    for &province_id in province_ids {
      ...
      if let Some(target_state_id) = target_state_id {
        self.validate_no_provincial_data_conflict(province_id, target_state_id, from_state_id)?;
      }
      deltas.push(ProvinceEditDelta {
        province_id,
        from_state_id,
        to_state_id: target_state_id,
        victory_points: self.working.victory_points_owned(from_state_id, province_id),
        province_buildings: self.working.province_buildings_for(from_state_id, province_id).cloned(),
      });
    }
    Ok(deltas)
  }
  ...
  fn snapshot_state(&self, state_id: u32) -> Result<WorkingStateSnapshot, StateEditError> {
    Ok(WorkingStateSnapshot {
      state_id,
      properties: self.working.properties_by_state.get(&state_id)
        .cloned()
        .ok_or(StateEditError::TargetStateNotFound(state_id))?,
      provinces: self.working.provinces_by_state.get(&state_id)
        .cloned()
        .unwrap_or_default(),
      victory_points: self.working.victory_points_by_state.get(&state_id)
        .cloned()
        .unwrap_or_default(),
      province_buildings: self.working.province_buildings_by_state.get(&state_id)
        .cloned()
        .unwrap_or_default(),
      origin: self.origins_by_state.get(&state_id)
        .cloned()
        .ok_or(StateEditError::TargetStateNotFound(state_id))?,
  ...
  fn validate_no_provincial_data_conflict(
    ...
    let incoming_vp = !self.working.victory_points_owned(from_state_id, province_id).is_empty();
    let target_vp = !self.working.victory_points_owned(Some(target_state_id), province_id).is_empty();
    let incoming_buildings = self.working.province_buildings_for(from_state_id, province_id);
    let target_buildings = self.working.province_buildings_for(Some(target_state_id), province_id);
  ...
  fn apply_command(&mut self, command: &StateEditCommand, direction: Direction) {
    match command {
      ...
      StateEditCommand::UpdateStateProperties { change } => {
        ...
        self.working.properties_by_state.insert(change.state_id, properties.clone());
      },
      ...
      StateEditCommand::RemoveState { state, deltas, .. } => match direction {
        ...
        Direction::Backward => {
          ...
          self.apply_province_deltas(deltas, Direction::Backward);
          self.working.provinces_by_state.insert(state.state_id, state.provinces.clone());
          self.working.victory_points_by_state.insert(state.state_id, state.victory_points.clone());
          self.working.province_buildings_by_state.insert(state.state_id, state.province_buildings.clone());
        },
      },
    }
  }
  ...
  fn restore_state(&mut self, state: &WorkingStateSnapshot) {
    self.working.properties_by_state.insert(state.state_id, state.properties.clone());
    self.working.provinces_by_state.insert(state.state_id, state.provinces.clone());
    self.working.victory_points_by_state.insert(state.state_id, state.victory_points.clone());
    self.working.province_buildings_by_state.insert(state.state_id, state.province_buildings.clone());
    ...
    self.origins_by_state.insert(state.state_id, state.origin.clone());
  ...
  fn remove_state_snapshot(&mut self, state: &WorkingStateSnapshot, tombstone: bool) {
    ...
    self.working.province_buildings_by_state.remove(&state.state_id);
    self.working.dated_history_states.remove(&state.state_id);
  ...
  fn recompute_dirty(&mut self) {
    self.dirty_state_ids.clear();
    let ids = self.baseline.provinces_by_state.keys()
    ...
    for id in ids {
      if self.baseline.provinces_by_state.get(&id) != self.working.provinces_by_state.get(&id)
        || self.baseline.properties_by_state.get(&id) != self.working.properties_by_state.get(&id)
        || !victory_points_match(
          self.baseline.victory_points_by_state.get(&id),
          self.working.victory_points_by_state.get(&id),
        )
        || self.baseline.province_buildings_by_state.get(&id) != self.working.province_buildings_by_state.get(&id)
      {
  ...
  fn push_command_diagnostics(&mut self, command: &StateEditCommand) {
    match command {
      StateEditCommand::ReassignProvinces { deltas } => for delta in deltas {
        ...
        if delta.to_state_id.is_none() {
          self.session_diagnostics.push(ProjectDiagnostic::new(
            ...
            None,
            None,
            format!("Province {} is currently unassigned due to an unsaved edit.", delta.province_id),
          ));
        }
        for state_id in [delta.from_state_id, delta.to_state_id].into_iter().flatten() {
      ...
      StateEditCommand::UpdateStateProperties { change } => {
        self.session_diagnostics.push(ProjectDiagnostic::new(
          ...
          None,
          None,
          format!("State {} properties were updated in memory.", change.state_id),
        ));
      },
      StateEditCommand::UpdateProvinceData {
        province_id,
        ...
        self.session_diagnostics.push(ProjectDiagnostic::new(
          ...
          None,
          None,
          format!("Province {province_id} data in State {state_id} was updated in memory."),
        ));
      },
      StateEditCommand::CreateState { state, deltas } => {
        self.session_diagnostics.push(ProjectDiagnostic::new(
          ...
          None,
          None,
          format!("State {} was created in memory and has no backing file.", state.state_id),
        ));
        if deltas.is_empty() {
          self.session_diagnostics.push(ProjectDiagnostic::new(
            ProjectDiagnosticKind::StateEditSession,
      ...
      StateEditCommand::RemoveState { state, policy, deltas } => {
        self.session_diagnostics.push(ProjectDiagnostic::new(
          ProjectDiagnosticKind::StateEditSession,
          DiagnosticSeverity::Info,
          None,
          None,
          format!("State {} is marked as removed in the current session.", state.state_id),
        ));
  ...
  fn description(&self) -> String {
    match self {
      ...
      Self::CreateState { state, deltas } => {
        format!("Created State {} with {} province(s)", state.state_id, deltas.len())
      },
      Self::RemoveState { state, deltas, .. } => {
        format!("Removed State {} with {} province(s)", state.state_id, deltas.len())
      },
    }
  }
}
...
impl StateWorkingSet {
  fn from_project(project: &Hoi4Project, province_kinds: &BTreeMap<u32, ProvinceKind>) -> Self {
    ...
    for (&province_id, &state_id) in &project.state_by_province {
      provinces_by_state.entry(state_id).or_default().insert(province_id);
    }
    ...
    for (&state_id, &document_index) in &project.states_by_id {
      let Some(data) = project.states.get(document_index).and_then(|document| document.data.as_ref()) else {
        continue;
      };
  ...
  pub fn state_data(&self, state_id: u32) -> StateData {
    let mut data = StateData {
      id: Some(state_id),
      provinces: self.provinces_by_state.get(&state_id).cloned().unwrap_or_default(),
      ..Default::default()
    };
    if let Some(properties) = self.properties_by_state.get(&state_id) {
      properties.apply_to(&mut data);
    }
    data.history.victory_points = self.victory_points_by_state.get(&state_id).cloned().unwrap_or_default();
    data.history.province_buildings = self.province_buildings_by_state.get(&state_id).cloned().unwrap_or_default();
    data
  }
  ...
  fn move_province(&mut self, province_id: u32, from: Option<u32>, to: Option<u32>) {
    ...
    if let Some(to) = to {
      self.provinces_by_state.entry(to).or_default().insert(province_id);
  ...
  fn province_data(
    ...
    EditableProvinceData {
      victory_point: self.victory_points_owned(state_id, province_id)
        .first()
        .map(|victory_point| victory_point.value),
      buildings: self.province_buildings_for(state_id, province_id)
        .cloned()
        .unwrap_or_default(),
    }
  }

  fn set_province_data(
    &mut self,
    state_id: u32,
    province_id: u32,
    ...
    let original_position = victory_points.iter()
    ...
    if let Some(value) = data.victory_point {
      let position = original_position.unwrap_or(victory_points.len()).min(victory_points.len());
      victory_points.insert(position, VictoryPoint { province_id, value });
    }
    let province_buildings = self.province_buildings_by_state.entry(state_id).or_default();
  ...
  fn move_victory_points(&mut self, delta: &ProvinceEditDelta, from: Option<u32>, to: Option<u32>) {
    Self::remove_victory_points(&mut self.victory_points_by_state, &mut self.detached_victory_points, from, delta.province_id);
    if delta.victory_points.is_empty() {
      return;
    }
    if let Some(to) = to {
      self.victory_points_by_state.entry(to).or_default().extend(delta.victory_points.clone());
    } else {
      self.detached_victory_points.insert(delta.province_id, delta.victory_points.clone());
    }
  }
  ...
  fn move_province_buildings(&mut self, delta: &ProvinceEditDelta, from: Option<u32>, to: Option<u32>) {
    ...
    if let Some(to) = to {
      self.province_buildings_by_state.entry(to).or_default().insert(delta.province_id, buildings);
    } else {
      self.detached_province_buildings.insert(delta.province_id, buildings);
    }
  }
...
impl fmt::Display for StateEditError {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    match self {
      ...
      StateEditError::ProvinceNotFound(id) => write!(f, "Province {id} does not exist in the map"),
      StateEditError::ProvinceIsNotLand(id) => write!(f, "Province {id} is not land and cannot be assigned to a state"),
      StateEditError::AmbiguousProvince(id) => write!(f, "Province {id} is ambiguous in the original state files"),
      StateEditError::ProvinceInInvalidState { province_id, state_id } => {
        write!(f, "Province {province_id} belongs to invalid state {state_id}")
      },
      ...
      StateEditError::StateIdReserved(id) => write!(f, "State ID {id} is already occupied or reserved by edit history"),
      StateEditError::StateAlreadyRemoved(id) => write!(f, "State {id} is already removed from this session"),
      ...
      StateEditError::TargetStateInvalid(id) => write!(f, "State {id} is not a valid edit target"),
      StateEditError::ProvincialDataConflict { province_id, target_state_id } => {
        write!(f, "Province {province_id} has conflicting provincial data in target state {target_state_id}")
      },
...
fn state_id_from_filename(path: &std::path::Path) -> Option<u32> {
  ...
  let digits = filename.chars().take_while(|character| character.is_ascii_digit()).collect::<String>();
...
mod tests {
  ...
  fn session() -> StateEditSession {
    ...
    project.states[0].data.as_mut().unwrap().history.victory_points.push(VictoryPoint { province_id: 10, value: 5 });
    project.states[0].data.as_mut().unwrap().history.province_buildings.insert(11, BTreeMap::from([("bunker".into(), 2)]));
    ...
    StateEditSession {
      ...
      origins_by_state: BTreeMap::from([
        (1, WorkingStateOrigin::Loaded { document_path: PathBuf::from("1.txt") }),
        (2, WorkingStateOrigin::Loaded { document_path: PathBuf::from("2.txt") }),
      ]),
      removed_state_ids: BTreeSet::new(),
      province_kinds,
  ...
  fn province_selection_rejects_sea_lake_and_ambiguous() {
    ...
    assert_eq!(edit.toggle_selected_province(40).unwrap_err(), StateEditError::ProvinceIsNotLand(40));
    assert_eq!(edit.toggle_selected_province(41).unwrap_err(), StateEditError::ProvinceIsNotLand(41));
    assert_eq!(edit.toggle_selected_province(12).unwrap_err(), StateEditError::AmbiguousProvince(12));
  }
  ...
  fn real_mod_state_edit_smoke_move_undo_redo_discard_unassign() {
    ...
    let invalid_documents = project.states.iter()
      .filter(|document| document.data.is_none())
      .map(|document| document.path.display().to_string())
      .collect::<Vec<_>>();
    let error_bearing_documents = project.states.iter()
      .filter(|document| document.diagnostics.iter().any(|diagnostic| {
        diagnostic.severity == DiagnosticSeverity::Error
      }))
      .map(|document| document.path.display().to_string())
      .collect::<Vec<_>>();
    let warning_kinds = project.diagnostics.iter()
      .filter(|diagnostic| diagnostic.severity == DiagnosticSeverity::Warning)
      .fold(BTreeMap::<String, usize>::new(), |mut counts, diagnostic| {
    ...
    for diagnostic in project.diagnostics.iter()
      .filter(|diagnostic| diagnostic.severity == DiagnosticSeverity::Error)
    {
      println!(
        "ERROR [{:?}] [{}] {}",
    ...
    let valverde_target = edit.valid_state_ids.iter()
      .copied()
    ...
    assert!(edit.update_province_data(5144, 1, valverde_after.clone()).unwrap());
    ...
    assert!(edit.redo());
    assert_eq!(edit.province_data(5144), Some(valverde_after.clone()));
    edit.reassign_provinces(&[5144], Some(valverde_target)).unwrap();
    assert_eq!(edit.province_data(5144), Some(valverde_after.clone()));
    assert!(edit.undo());
    assert!(edit.undo());
    assert_eq!(edit.province_data(5144).unwrap().victory_point, Some(5));
    ...
    assert_eq!(edit.province_state_id(5144), Some(1));
    assert!(edit.redo());
    edit.remove_state(suggested_state_id, StateRemovalPolicy::MoveToState(1)).unwrap();
    assert_eq!(edit.province_state_id(5144), Some(1));
    assert_eq!(edit.province_data(5144).unwrap().victory_point, Some(5));
    assert!(!edit.is_dirty());
    edit.discard();
    ...
    let candidate = bundle.map.iter_boundaries().find_map(|(boundary, _)| {
      ...
      probe.reassign_provinces(&[source_id], Some(target_state)).ok()?;
      Some((source_id, source_state, target_id, target_state, source_pos, target_pos))
    ...
    assert_eq!(edit.state_by_province().get(&source_id), Some(&target_state));
    let move_visual = measure_visual(&edit);
    assert!(edit.undo());
    assert_eq!(edit.state_by_province().get(&source_id), Some(&source_state));
  ...
  fn state_edit_moves_victory_points_and_undo_restores_them() {
    ...
    assert_eq!(edit.working.victory_points_by_state[&2], vec![VictoryPoint { province_id: 10, value: 5 }]);
    assert!(edit.undo());
    assert_eq!(edit.working.victory_points_by_state[&1], vec![VictoryPoint { province_id: 10, value: 5 }]);
  }
  ...
  fn state_edit_moves_province_buildings_and_undo_restores_them() {
    ...
    assert_eq!(edit.working.province_buildings_by_state[&2][&11]["bunker"], 2);
    assert!(edit.undo());
  ...
  fn state_edit_conflicting_provincial_data_blocks_whole_command() {
    let mut edit = session();
    edit.working.province_buildings_by_state.entry(2).or_default().insert(11, BTreeMap::from([("dockyard".into(), 1)]));
    let before = edit.working.clone();
    assert_eq!(
      edit.reassign_provinces(&[10, 11], Some(2)).unwrap_err(),
      StateEditError::ProvincialDataConflict { province_id: 11, target_state_id: 2 }
    );
    assert_eq!(edit.working, before);
    assert!(edit.undo_stack.is_empty());
  }
  ...
  fn state_lifecycle_create_remove_history_and_data_roundtrip() {
    let mut edit = session();
    assert_eq!(state_id_from_filename(std::path::Path::new("111-State_111.txt")), Some(111));
    ...
    assert_eq!(edit.state_origin(112), Some(&WorkingStateOrigin::CreatedInSession));
    ...
    assert!(edit.redo());
    assert_eq!(edit.state_data(112).unwrap().name.as_deref(), Some("Temporary State"));

    edit.remove_state(112, StateRemovalPolicy::MoveToState(1)).unwrap();
    assert_eq!(edit.state_lifecycle(112), Some(WorkingStateLifecycle::RemovedInSession));
    assert_eq!(edit.province_state_id(10), Some(1));
    assert_eq!(edit.province_data(10).unwrap().victory_point, Some(5));
    assert!(!edit.is_dirty());
    assert!(edit.undo());
    ...
    edit.remove_state(1, StateRemovalPolicy::Unassign).unwrap();
    assert_eq!(edit.state_lifecycle(1), Some(WorkingStateLifecycle::RemovedInSession));
    assert_eq!(edit.province_state_id(10), None);
    assert_eq!(edit.province_data(10).unwrap().victory_point, Some(5));
    assert!(edit.undo());
    ...
    conflict.working.province_buildings_by_state.entry(2).or_default()
      .insert(11, BTreeMap::from([("dockyard".into(), 1)]));
    ...
    reservation.create_state(3, properties.clone(), false).unwrap();
