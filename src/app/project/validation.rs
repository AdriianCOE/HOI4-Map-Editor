...
impl SemanticDifference {
    fn describe(&self) -> String {
        match self {
            ...
            Self::ProvinceAssignmentMismatch {
                province_id,
                expected,
                actual,
            } => {
...
impl CombinedRoundTripValidationReport {
    fn new(round_trip: RoundTripValidationReport, candidate_digest: SourceFingerprint) -> Self {
        let authorization_digest =
            combined_authorization_digest(&round_trip, &candidate_digest, None);
        Self {
            round_trip,
    ...
    fn with_project_validation(mut self, project_validation: ProjectValidationReport) -> Self {
        self.authorization_digest = combined_authorization_digest(
            &self.round_trip,
...
impl RoundTripValidator {
        ...
        F: FnMut(RoundTripStage),
    {
        ...
        let started = Instant::now();
        report.byte_comparison = gate!(compare_bytes(
            project,
            plan,
            &workspace.candidate,
            &snapshot,
        ...
        V: FnOnce(&Path) -> Result<(), String>,
    {
        ...
        let mut report = RoundTripValidationReport::new(plan);
        let mut workspace_root = None;
        let map_candidate_files = map_candidate_files.filter(|files| !files.is_empty());
        ...
        if let Some(files) = map_candidate_files {
            for relative in files.keys() {
                plan_sets
                    .modified
                    .insert(PathBuf::from("map").join(relative));
            }
        }
        ...
        report.timings.source_verification_ms = started.elapsed().as_millis();
        report.source_verification.files_verified = snapshot.files.len();
        report.source_verification.map_files_verified = snapshot
            .files
            .keys()
            .filter(|path| path.starts_with("map"))
            .count();
        report.source_verification.planned_sources_verified =
            plan.modified_files.len() + plan.removed_files.len();
        ...
        report.source_verification.source_unchanged_after_validation = true;
        report.status =
            if plan_sets.has_review_required || project_validation.delta.has_review_items() {
                RoundTripStatus::PassedWithReview
            } else {
                RoundTripStatus::Passed
            };
...
fn validate_plan(
    ...
    for (kind, path, safety) in plan
        ...
        .chain(
            plan.created_files
                .iter()
                .map(|file| ("created", &file.path, file.safety)),
        )
        .chain(
            plan.removed_files
                .iter()
...
fn validate_state_relative_path(path: &Path) -> Result<(), Failure> {
    ...
    if components.iter().any(|component| {
        ...
        return Err(unsafe_path(
            path,
            "path traversal or drive prefixes are not allowed",
        ));
    }
...
fn apply_plan(
    ...
    for file in &plan.modified_files {
        ...
        let before = read(
            &target,
            RoundTripStage::Applying,
            "read candidate before modification",
        )?;
        ...
        let applied = apply_operations(&before, &file.operations).map_err(|message| {
            Failure::new(
                RoundTripStage::Applying,
                Some(target.clone()),
                Some(file.state_id),
    ...
    for file in &plan.created_files {
        ...
        if manifest
            .copied_files
            .keys()
            .any(|path| path_key(path) == path_key(&file.path))
        {
            return Err(Failure::new(
                RoundTripStage::Applying,
                Some(target),
...
fn validate_map_candidate_paths(files: &BTreeMap<PathBuf, Vec<u8>>) -> Result<(), Failure> {
    for relative in files.keys() {
            ...
            )
        {
            return Err(unsafe_path(
                relative,
                "candidate map paths must be direct province-map output files",
            ));
        }
    }
    Ok(())
...
fn apply_map_candidate(
    ...
    for (relative, bytes) in files {
        let target =
            resolve_map_or_state_path(candidate_root, &PathBuf::from("map").join(relative))?;
        if target.exists() {
            write_existing(
                &target,
                bytes,
...
fn verify_final_file_set(
    ...
    let expected = baseline
        .difference(&manifest.expected_removed_files)
        .cloned()
...
pub(crate) fn reload_project_for_save(
    ...
    reload_project(project_root).map_err(|failure| {
        format!(
            "{} {}",
            failure.diagnostic.message, failure.diagnostic.action
        )
    })
}
...
fn compare_semantics(
    ...
    let expected_ids = source
        ...
        .collect::<BTreeSet<_>>();
    let actual_ids = candidate
        .states_by_id
        .keys()
        .copied()
        .collect::<BTreeSet<_>>();
    let mut differences = expected_ids
        ...
        .chain(
            actual_ids
                ...
                .copied()
                .map(SemanticDifference::UnexpectedState),
        )
        .collect::<Vec<_>>();
    ...
    let source_non_land = source
        ...
        .copied()
        .collect::<BTreeSet<_>>();
    let working_non_land = edit
        .state_by_province()
        .keys()
        .filter(|province_id| !land_province_ids.contains(province_id))
        .copied()
        .collect::<BTreeSet<_>>();
    let province_coverage_match =
        coverage == *land_province_ids && working_non_land == source_non_land;
    let removed_states_match = plan
        .removed_files
        .iter()
        .all(|file| !candidate.states_by_id.contains_key(&file.state_id));
    let created_states_match = plan
        .created_files
        .iter()
        .all(|file| candidate.states_by_id.contains_key(&file.state_id));
...
fn compare_bytes_with_map_candidate(
    ...
    for relative in snapshot.files.keys() {
        ...
        let source = read(
            &source_path,
            RoundTripStage::ByteComparison,
            "read source for byte comparison",
        )?;
        ...
        if manifest.expected_modified_files.contains(relative) {
            ...
            if &candidate != expected {
                result
                    .differences
                    .push(byte_difference(relative, expected, &candidate));
            }
            ...
            if candidate != source {
                result
                    .differences
                    .push(byte_difference(relative, &source, &candidate));
            }
    ...
    for file in &plan.created_files {
        ...
        let candidate = read(
            &candidate_path,
            RoundTripStage::ByteComparison,
            "read created candidate",
        )?;
        if candidate != file.content {
            result
                .differences
                .push(byte_difference(&file.path, &file.content, &candidate));
        }
    ...
    if let Some(files) = map_candidate_files {
        for (relative, expected) in files {
            ...
            let candidate = read(
                &candidate_path,
                RoundTripStage::ByteComparison,
                "read created map candidate",
            )?;
            if &candidate != expected {
                result
                    .differences
                    .push(byte_difference(&project_relative, expected, &candidate));
            }
            result.created_files_verified += 1;
        }
    }
    result.map_files_unchanged = result
        .differences
        .iter()
    ...
    if !result.differences.is_empty() {
        ...
        return Err(Failure::new(
            ...
            format!(
                ...
                first.first_different_offset, first.source_len, first.candidate_len,
            ),
...
fn compare_diagnostics(
    ...
    let unexpected_structural = candidate_structural
        .iter()
        .map(|(key, &count)| count.saturating_sub(*source_structural.get(key).unwrap_or(&0)))
        .sum::<usize>();
    let candidate_unassigned = candidate
        .diagnostics
        .iter()
    ...
    let planned_paths = plan
        ...
        .collect::<BTreeSet<_>>();
    let expected_session_diagnostics = candidate
        .diagnostics
        .iter()
        .filter(|diagnostic| {
...
fn diagnostic_counts(
    ...
    for diagnostic in &project.diagnostics {
        ...
        *counts
            .entry((diagnostic.kind, diagnostic.severity))
            .or_default() += 1;
    }
    counts
}
...
fn verify_source_unchanged(
    ...
    for (relative, fingerprint) in &snapshot.files {
        ...
        let bytes = read(
            &path,
            RoundTripStage::SourceVerification,
            "re-read source after validation",
        )?;
        if FileFingerprint::from_bytes(&bytes) != *fingerprint {
            return Err(Failure::new(
                RoundTripStage::SourceVerification,
...
fn resolve_map_or_state_path(candidate_root: &Path, relative: &Path) -> Result<PathBuf, Failure> {
    if is_state_path(relative) {
        ...
        Err(unsafe_path(
            relative,
            "only province-map outputs and direct state files are supported",
        ))
    }
}

fn candidate_state_path(candidate_root: &Path, relative: &Path) -> Result<PathBuf, Failure> {
    resolve_candidate_path(candidate_root, relative).map_err(|message| {
        Failure::new(
            RoundTripStage::Preflight,
...
fn enumerate_candidate_state_paths(candidate_root: &Path) -> Result<BTreeSet<PathBuf>, Failure> {
    ...
    let entries = fs::read_dir(&states).map_err(|err| {
        Failure::io(
            RoundTripStage::FilesystemVerification,
...
fn byte_difference(path: &Path, expected: &[u8], actual: &[u8]) -> ByteDifference {
    let first_different_offset = expected
        .iter()
        .zip(actual)
...
mod tests {
    ...
    fn source_change_is_detected_before_workspace_creation() {
        ...
        fs::write(
            ...
            "state={id=1 provinces={1} manpower=2}",
        )
        .unwrap();
        let report = RoundTripValidator::default().validate(
            &project,
    ...
    fn stale_cancelled_and_review_results_are_distinct() {
        ...
        cancellation.cancel();
        let cancelled =
            RoundTripValidator::default().validate(&project, &edit, &plan, &cancellation, |_| {});
    ...
    fn created_and_removed_states_exist_only_in_candidate() {
        ...
        remove_edit
            .remove_state(1, super::super::StateRemovalPolicy::Unassign)
            .unwrap();
        ...
        assert_eq!(
            remove_report.status,
            RoundTripStatus::Passed,
            "{}",
            remove_report.full_text()
        );
    ...
    fn combined_candidate_overlays_map_and_states_without_touching_source() {
        ...
        let report = RoundTripValidator::default().validate_combined(
            ...
            |map_root| {
                let bytes =
                    fs::read(map_root.join("definition.csv")).map_err(|err| err.to_string())?;
    ...
    fn test_map() -> crate::app::map::Map {
        ...
        let map = Bundle::load(&Location::Directory(paths.map_directory), config)
            .unwrap()
            .map;
        fs::remove_dir_all(root).unwrap();
        map
