...
impl ProjectPaths {
  pub fn discover(root: impl Into<PathBuf>) -> Result<Self, ProjectPathError> {
    let root = root.into();
    require_directory(&root, ProjectPathError::RootNotFound, ProjectPathError::RootIsNotDirectory)?;

    let map_directory = root.join("map");
    require_directory(&map_directory, ProjectPathError::MissingMapDirectory, ProjectPathError::MissingMapDirectory)?;
    ...
    require_directory(&history_directory, ProjectPathError::MissingHistoryDirectory, ProjectPathError::MissingHistoryDirectory)?;

    let states_directory = history_directory.join("states");
    require_directory(&states_directory, ProjectPathError::MissingStatesDirectory, ProjectPathError::MissingStatesDirectory)?;
...
fn require_file(path: &Path, missing: fn(PathBuf) -> ProjectPathError) -> Result<(), ProjectPathError> {
  match path.metadata() {
    Ok(metadata) if metadata.is_file() => Ok(()),
    Ok(_) => Err(missing(path.to_owned())),
    Err(err) if err.kind() == io::ErrorKind::NotFound => Err(missing(path.to_owned())),
    Err(source) => Err(ProjectPathError::Io { path: path.to_owned(), source })
  }
}
...
mod tests {
  ...
  fn discovers_valid_project_root() {
    ...
    assert_eq!(paths.states_directory, project.path().join("history/states"));
  }
