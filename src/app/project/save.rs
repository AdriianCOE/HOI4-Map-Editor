...
use super::{
  Hoi4Project, PatchSafety, ProjectPatchPlan, RoundTripStatus,
...
impl StateSaveBlockReason {
  pub fn message(self) -> &'static str {
    match self {
      ...
      Self::PatchPlanStale => "The patch preview is outdated. Regenerate and validate it again.",
      Self::NoChanges => "There are no state changes to save.",
      Self::ReviewRequired => {
        "Review-required patches cannot be saved automatically."
      },
      Self::BlockedPatch => "Blocked patches cannot be saved.",
      Self::NoRoundTripReport => {
        "Run the isolated round-trip validation before saving."
      },
...
pub fn state_save_eligibility(
  ...
  if plan.summary.blocked_files != 0
    || plan.modified_files.iter().any(|file| file.safety == PatchSafety::Blocked)
    || plan.created_files.iter().any(|file| file.safety == PatchSafety::Blocked)
    || plan.removed_files.iter().any(|file| file.safety == PatchSafety::Blocked)
  {
    reasons.push(StateSaveBlockReason::BlockedPatch);
  }
  if plan.summary.review_required_files != 0
    || plan.modified_files.iter().any(|file| file.safety == PatchSafety::ReviewRequired)
    || plan.created_files.iter().any(|file| file.safety == PatchSafety::ReviewRequired)
    || plan.removed_files.iter().any(|file| file.safety == PatchSafety::ReviewRequired)
  {
  ...
  let authorization = StateSaveAuthorization {
    ...
    source_fingerprints: plan.source_fingerprints.iter()
...
pub fn save_confirmation_text(
  ...
  format!(
    ...
    project.paths.root.join(INTERNAL_DIRECTORY).join("backups").display(),
  )
}
...
pub fn execute_state_save(
  ...
  let mut journal = SaveTransactionJournal {
    ...
    operations: resolved.iter().map(ResolvedOperation::journal_entry).collect(),
  ...
  let map_snapshot = match snapshot_map_files(project) {
    ...
    Err(error) => return fail_before_commit(
      &mut journal,
      &journal_path,
      &lock_path,
      &resolved,
  ...
  if let Err(error) = transition(
    ...
    "backup started",
  ).and_then(|_| create_and_verify_backup(
    &backup_directory,
  ...
  match commit_operations(&resolved, &mut journal, &journal_path, fault, &mut progress) {
    ...
    Err(error) => return rollback_after_failure(
      &mut journal,
      &journal_path,
      &lock_path,
      &resolved,
  ...
  progress(SaveTransactionState::ValidatingSavedProject, resolved.len(), resolved.len());
  if let Err(error) = transition(
    &mut journal,
    &journal_path,
  ...
  let reloaded_project = match validate_saved_project(
    &root,
    project,
    edit,
    plan,
  ...
  let _ = fs::remove_file(&lock_path);
  progress(SaveTransactionState::Completed, resolved.len(), resolved.len());
  StateSaveReport {
...
impl ResolvedOperation {
  ...
  fn stage_path(&self) -> Option<&Path> {
    match self {
      Self::Modify { stage_path, .. } | Self::Create { stage_path, .. } => {
        Some(stage_path)
      },
      Self::Remove { .. } => None,
    }
  ...
  fn journal_entry(&self) -> SaveFileOperationJournal {
    SaveFileOperationJournal {
      ...
      original_fingerprint: self.before()
        .map(SourceFingerprint::from_bytes)
        .as_ref()
        .map(Into::into),
      candidate_fingerprint: self.candidate()
        .map(SourceFingerprint::from_bytes)
        .as_ref()
        .map(Into::into),
...
fn resolve_final_path(root: &Path, relative: &Path) -> Result<PathBuf, String> {
  ...
  if final_path.parent() != Some(canonical_states.as_path()) {
    return Err(format!("State path escaped the states directory: {}", relative.display()));
  }
  Ok(final_path)
}
...
fn ensure_unique_path(paths: &mut BTreeSet<String>, path: &Path) -> Result<(), String> {
  ...
  if paths.insert(normalized) {
    ...
    Err(format!("Case-insensitive state path collision: {}", path.display()))
  }
}
...
fn ensure_writable_existing(path: &Path) -> Result<(), String> {
  ...
  if link_metadata.file_type().is_symlink() {
    return Err(format!("Symbolic links are not accepted for state files: {}", path.display()));
  }
...
fn internal_sibling(
  ...
  Ok(final_path.with_file_name(format!(
    "{filename}.hse-{purpose}-{transaction_id}"
  )))
}
...
fn validate_authorization(
  ...
  {
    return Err(
      "The current patch plan is not the plan authorized for Save.".to_owned()
    );
  }
  if report.status != RoundTripStatus::Passed
    || !report.eligible_for_atomic_save_preparation
...
fn verify_source(project: &Hoi4Project, plan: &ProjectPatchPlan) -> Result<(), String> {
  ...
  for file in &plan.created_files {
    ...
    if !planned_paths.insert(normalized) {
      return Err(format!("Case-insensitive path collision: {}", file.path.display()));
    }
    let final_path = resolve_final_path(&root, &file.path)?;
  ...
  for (source, fingerprint) in &plan.source_fingerprints {
    ...
    if SourceFingerprint::from_bytes(&bytes) != *fingerprint {
      return Err(format!("Source changed after validation: {}", source.display()));
    }
  }
  Ok(())
}
...
fn verify_exact_file(path: &Path, expected: &[u8]) -> Result<(), String> {
  ...
  if actual == expected {
    ...
    Err(format!("Source changed after validation: {}", path.display()))
  }
}
...
fn create_and_verify_backup(
  ...
  for operation in operations {
    ...
    entries.push(BackupManifestEntry {
      ...
      original_fingerprint: operation.before()
        .map(SourceFingerprint::from_bytes)
        .as_ref()
        .map(Into::into),
      candidate_fingerprint: operation.candidate()
        .map(SourceFingerprint::from_bytes)
        .as_ref()
        .map(Into::into),
...
fn verify_backup(manifest_path: &Path) -> Result<(), String> {
  ...
  let backup_root = manifest_path.parent()
    ...
    match (&entry.original_fingerprint, &entry.backup_path) {
      (Some(expected), Some(path)) => {
        let canonical = dunce::canonicalize(path)
          .map_err(|error| format!("Cannot canonicalize backup {}: {error}", path.display()))?;
        if !canonical.starts_with(&canonical_backup_root) {
          return Err(format!("Backup escaped its directory: {}", canonical.display()));
        }
        let bytes = fs::read(&canonical)
          .map_err(|error| format!("Cannot verify backup {}: {error}", canonical.display()))?;
...
fn stage_candidates(
  ...
  for (index, operation) in operations.iter().enumerate() {
    let Some(stage_path) = operation.stage_path() else { continue };
    ...
    let bytes = operation.candidate()
      .ok_or_else(|| format!("Missing candidate bytes for {}", operation.relative().display()))?;
    ...
    if verified != bytes {
      return Err(format!("Staged bytes differ for {}", operation.relative().display()));
    }
    ...
    match operation {
      ResolvedOperation::Modify {
        ...
        fs::rename(final_path, rollback_path)
          .map_err(|error| rename_error("move destination to rollback", final_path, error))?;
      ...
      ResolvedOperation::Remove {
        ...
        ensure_absent(rollback_path)?;
        fs::rename(final_path, rollback_path)
          .map_err(|error| rename_error("move removed state to rollback", final_path, error))?;
        journal.operations[index].progress = FileOperationProgress::Removed;
      },
    }
    write_journal(journal_path, journal)?;
    progress(SaveTransactionState::Committing, index + 1, operations.len());
    if fault == StateSaveFault::FailAfterCommit(index + 1) {
      return Err(format!("Injected commit failure after {} operation(s)", index + 1));
    }
...
fn verify_operation_source(operation: &ResolvedOperation) -> Result<(), String> {
  match operation {
    ResolvedOperation::Modify { final_path, before, .. }
    | ResolvedOperation::Remove { final_path, before, .. } => {
...
fn validate_saved_project(
  ...
  for operation in operations {
    match operation {
      ResolvedOperation::Modify { final_path, after, .. } => {
        verify_exact_file(final_path, after)?;
      },
      ResolvedOperation::Create { final_path, content, .. } => {
        verify_exact_file(final_path, content)?;
      },
      ResolvedOperation::Remove { final_path, .. } => {
        if final_path.exists() {
          return Err(format!("Removed state still exists: {}", final_path.display()));
        }
      },
    }
  }
...
fn rollback_operations(
  ...
  for (index, operation) in operations.iter().enumerate().rev() {
    ...
    if let Err(error) = remove_if_exists(operation.stage_path()) {
      errors.push(format!("stage cleanup for {}: {error}", operation.relative().display()));
    }
  }
  errors
}
...
fn restore_modified(
  ...
  if rollback_path.exists() {
    if final_path.exists() {
      verify_exact_file(final_path, candidate)
        .map_err(|_| format!(
...
fn rollback_created(
  ...
  if final_path.exists() {
    ...
    fs::remove_file(final_path)
      .map_err(|error| format!("Cannot remove created state {}: {error}", final_path.display()))
  } else if matches!(
    progress,
    FileOperationProgress::Pending | FileOperationProgress::Staged
      | FileOperationProgress::CandidateInstalled | FileOperationProgress::RolledBack
  ) {
    Ok(())
  } else {
    Err(format!("Unexpected create rollback state for {}", final_path.display()))
  }
}
...
fn fail_before_commit(
  ...
  StateSaveReport {
    ...
    backup_path: backup_directory.exists().then(|| backup_directory.to_owned()),
    journal_path: Some(journal_path.to_owned()),
    modified_files: counts.0,
    created_files: counts.1,
    removed_files: counts.2,
...
pub fn recover_interrupted_state_save(root: &Path) -> StateSaveReport {
  ...
  let counts = journal.operations.iter().fold((0, 0, 0), |mut counts, operation| {
    match operation.kind {
  ...
  if let Err(error) = verify_backup(&journal.backup_manifest_path) {
    errors.push(format!("Backup verification failed before recovery: {error}"));
  }
  ...
  if errors.is_empty() {
    ...
    let _ = fs::remove_file(&lock_path);
    let reloaded_project = reload_project_for_save(&root).ok().map(|(project, _)| project);
    StateSaveReport {
      outcome: StateSaveOutcome::RolledBack,
      transaction_id: Some(journal.transaction_id),
      state: SaveTransactionState::RolledBack,
...
fn rollback_journal_operation(
  ...
  match operation.kind {
    SaveFileKind::Modify => {
      let rollback = operation.rollback_path.as_deref()
      ...
      if rollback.exists() {
        if final_path.exists() {
          ...
          fs::remove_file(final_path).map_err(|error| format!(
            "Cannot remove failed candidate {}: {error}",
            final_path.display()
          ))?;
        }
...
fn verify_persisted_file(
  ...
  if expected.matches(&bytes) {
    ...
    Err(format!("{description} fingerprint mismatch for {}", path.display()))
  }
}
...
fn valid_transition(from: SaveTransactionState, to: SaveTransactionState) -> bool {
  ...
  from == to || matches!(
    ...
    (Preparing, VerifyingSource | FailedBeforeCommit | Cancelled)
      ...
      | (Committing, ValidatingSavedProject | RollingBack | RecoveryRequired)
      | (ValidatingSavedProject, Completed | RollingBack | RecoveryRequired)
...
fn snapshot_map_files(project: &Hoi4Project) -> Result<Vec<(PathBuf, Vec<u8>)>, String> {
  [
    ...
    fs::read(&path)
      ...
      .map_err(|error| format!("Cannot snapshot read-only map file {}: {error}", path.display()))
  }).collect()
}
...
fn create_lock(path: &Path, lock: &SaveLock) -> Result<(), String> {
  ...
  fs::create_dir_all(parent)
    .map_err(|error| format!("Cannot create {}: {error}", parent.display()))?;
  let bytes = toml::to_vec(lock)
    .map_err(|error| format!("Cannot serialize save lock: {error}"))?;
  ...
  let bytes = match fs::read(path) {
    ...
    Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
      let filename = path.file_name()
        ...
        .to_string_lossy();
      let previous = path.with_file_name(format!("{filename}.previous"));
      fs::read(&previous).map_err(|fallback| format!(
...
fn ensure_absent(path: &Path) -> Result<(), String> {
  if path.exists() {
    Err(format!("Internal transaction path already exists: {}", path.display()))
  } else {
    Ok(())
  }
}
...
fn canonical_root(root: &Path) -> Result<PathBuf, String> {
  dunce::canonicalize(root)
    .map_err(|error| format!("Cannot canonicalize project root {}: {error}", root.display()))
}
...
mod tests {
  ...
  fn exact_passed_report_is_required_for_save() {
    ...
    assert!(eligibility.reasons.contains(&StateSaveBlockReason::ReviewRequired));
    assert!(eligibility.reasons.contains(&StateSaveBlockReason::RoundTripNotPassed));
    cleanup(&root);
  }
  ...
  fn net_zero_plan_creates_no_save_metadata() {
    ...
    assert!(!eligibility.eligible);
    assert!(eligibility.reasons.contains(&StateSaveBlockReason::NoChanges));
  ...
  fn source_change_invalidates_authorization_before_backup() {
    ...
    assert!(!eligibility.eligible);
    assert!(eligibility.reasons.contains(&StateSaveBlockReason::SourceChanged));
    assert!(!root.join(INTERNAL_DIRECTORY).exists());
    cleanup(&root);
  }
  ...
  fn modified_state_save_creates_verified_backup_and_new_baseline_candidate() {
    ...
    assert_eq!(result.outcome, StateSaveOutcome::Completed, "{}", result.summary_text());
  ...
  fn mid_commit_and_post_validation_failures_restore_original_bytes() {
    for (name, fault) in [
      ...
      assert_eq!(result.outcome, StateSaveOutcome::RolledBack, "{}", result.summary_text());
      assert_eq!(fs::read(&state_path).unwrap(), before);
      assert!(result.backup_path.as_ref().is_some_and(|path| path.exists()));
      assert!(!root.join(INTERNAL_DIRECTORY).join(LOCK_FILE).exists());
      assert_no_transaction_siblings(&root);
      cleanup(&root);
    }
  ...
  fn interrupted_commit_is_detected_and_recovered_from_journal() {
    ...
    assert_eq!(recovered.outcome, StateSaveOutcome::RolledBack, "{}", recovered.summary_text());
    assert_eq!(fs::read(&state_path).unwrap(), before);
  ...
  fn completed_save_recovery_finishes_temporary_cleanup() {
    ...
    assert_eq!(saved.outcome, StateSaveOutcome::Completed, "{}", saved.summary_text());
    ...
    let recovered = recover_interrupted_state_save(&root);
    assert_eq!(recovered.outcome, StateSaveOutcome::Completed, "{}", recovered.summary_text());
  ...
  fn created_and_removed_files_follow_manifest_and_commit_rules() {
    ...
    assert_eq!(created.outcome, StateSaveOutcome::Completed, "{}", created.summary_text());
    ...
    remove_edit.remove_state(1, StateRemovalPolicy::Unassign).unwrap();
    ...
    assert_eq!(removed.outcome, StateSaveOutcome::Completed, "{}", removed.summary_text());
    assert!(!removed_path.exists());
    let manifest: BackupManifest =
      read_toml(&removed.backup_path.unwrap().join("manifest.toml")).unwrap();
    let backup = manifest.entries.iter()
  ...
  fn real_mod_phase4c_transactional_save_smoke_on_controlled_copies() {
    ...
    assert_eq!(original_snapshot.len(), 513, "expected 2 map files + 511 states");
    ...
    with_real_copy(&original, "b-province-5144", |root, project, mut edit| {
      ...
      let target = edit.valid_state_ids().iter().copied().find(|id| *id != 1).unwrap();
      ...
      let state = reloaded.state_document(target).unwrap().data.as_ref().unwrap();
      assert!(state.history.victory_points.iter().any(|vp| {
        vp.province_id == 5144 && vp.value == 5
      }));
      assert_promotable_clean_baseline(saved, root);
    });

    with_real_copy(&original, "c-create-512", |root, project, mut edit| {
      assert_eq!(edit.suggest_next_state_id(), 512);
      ...
      assert!(saved.reloaded_project.as_ref().unwrap().state_document(512).is_some());
      let manifest: BackupManifest =
    ...
    with_real_copy(&original, "d-remove", |root, project, mut edit| {
      ...
      assert_eq!(saved.outcome, StateSaveOutcome::Completed, "{}", saved.summary_text());
      assert!(!removed_path.exists());
      assert!(saved.reloaded_project.as_ref().unwrap().state_document(removal_id).is_none());
      assert_no_transaction_siblings(root);
      assert_promotable_clean_baseline(saved, root);
    });
    ...
    with_real_copy(&original, "e-combined", |root, project, mut edit| {
      ...
      let removal_id = edit.valid_state_ids().iter().copied()
        .filter(|id| *id != 1 && *id != 512)
        .find(|id| {
          let mut probe = edit.clone();
          probe.remove_state(*id, StateRemovalPolicy::MoveToState(1)).is_ok()
        })
        .unwrap();
      edit.remove_state(removal_id, StateRemovalPolicy::MoveToState(1)).unwrap();
      let plan = plan_state_patches(&project, &edit);
    ...
    for (label, fault, expected) in [
      ("f-staging-failure", StateSaveFault::FailStagingAt(0), StateSaveOutcome::FailedBeforeCommit),
      ("g-mid-commit", StateSaveFault::FailAfterCommit(1), StateSaveOutcome::RolledBack),
      ("h-post-validation", StateSaveFault::FailPostValidation, StateSaveOutcome::RolledBack),
    ] {
    ...
    with_real_copy(&original, "i-recovery", |root, project, mut edit| {
      ...
      edit.update_state_properties(1, properties).unwrap();
      let interrupted =
        save_real(&project, &edit, StateSaveFault::LeaveInterruptedAfterCommit(1));
      assert_eq!(interrupted.outcome, StateSaveOutcome::RecoveryRequired);
      assert!(detect_state_save_recovery(root).is_some());
      let recovered = recover_interrupted_state_save(root);
      assert_eq!(recovered.outcome, StateSaveOutcome::RolledBack, "{}", recovered.summary_text());
  ...
  fn copy_real_project(original: &Path, destination: &Path) {
    ...
    let mut states = fs::read_dir(original.join("history/states")).unwrap()
      .map(|entry| entry.unwrap().path())
      .filter(|path| path.is_file() && path.extension().is_some_and(|ext| {
    ...
    for source in states {
      fs::copy(
        &source,
        destination.join("history/states").join(source.file_name().unwrap()),
      ).unwrap();
    }
  }
  ...
  fn snapshot_original(root: &Path) -> BTreeMap<PathBuf, SourceFingerprint> {
    ...
    let mut states = fs::read_dir(root.join("history/states")).unwrap()
      .map(|entry| entry.unwrap().path())
      .filter(|path| path.is_file() && path.extension().is_some_and(|ext| {
        ext.eq_ignore_ascii_case("txt")
      }))
  ...
  fn removable_state(edit: &StateEditSession) -> u32 {
    edit.valid_state_ids().iter().copied()
      .filter(|id| *id != 1)
      .find(|id| {
        let mut probe = edit.clone();
        probe.remove_state(*id, StateRemovalPolicy::MoveToState(1)).is_ok()
  ...
  fn assert_promotable_clean_baseline(mut saved: StateSaveReport, root: &Path) {
    ...
    let bundle =
      Bundle::load(&Location::Directory(paths.map_directory), config).unwrap();
  ...
  fn authorize(
    ...
    state_save_eligibility(
      ...
      Some(report),
      StateSaveConditions::default(),
    ).authorization.expect("safe current plan should be authorized")
  }
