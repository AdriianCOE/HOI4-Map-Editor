...
use crate::app::state::{
    NewlineStyle, PdxBlock, PdxDocument, PdxEntry, PdxValue, SourceText, StateData, StateDocument,
    TextSpan, TokenKind, VictoryPoint, extract_state, parse,
};
...
pub fn plan_state_patches(project: &Hoi4Project, edit: &StateEditSession) -> ProjectPatchPlan {
    ...
    for &state_id in &removed_state_ids {
        let Some(origin) = edit.state_origin(state_id) else {
            continue;
        };
        if matches!(origin, WorkingStateOrigin::CreatedInSession) {
            continue;
        }
        let Some(document) = project.state_document(state_id) else {
            continue;
        };
    ...
    for &state_id in &dirty_state_ids {
        ...
        match edit.state_origin(state_id) {
            ...
            Some(WorkingStateOrigin::Loaded { .. }) => {
                ...
                let Some(baseline) = document.data.as_ref() else {
                    continue;
                };
                let Some(working) = edit.state_data(state_id) else {
                    continue;
                };
...
fn finish_modification(
    ...
    let mut safety = maximum_safety(
        builder
            .diagnostics
            .iter()
            .map(|diagnostic| diagnostic.safety)
...
fn plan_optional_scalar<T: PartialEq + Copy>(
    ...
    if bindings.len() > 1 {
        builder.diagnostic(
            ...
            format!(
                "State {} has multiple {field} assignments.",
                builder.state_id
            ),
    ...
    match (bindings.first().copied(), after) {
        ...
        (None, Some(value)) => {
            let (offset, content) =
                assignment_insertion(builder.document, root, field, &render(value));
            builder.insert(
                field,
                offset,
                content,
...
fn plan_bool(
    ...
    builder
        .changes
        .push(format!("impassable: {before} -> {after}"));
...
fn plan_named_block(
    ...
    let Some(container) = containers.first().copied() else {
        ...
        builder.changes.push(format!(
            "{field}: create block with {} entries",
            after.len()
        ));
        return;
    };
    ...
    for id in baseline.provinces.difference(&working.provinces) {
        ...
        if safety == PatchSafety::ReviewRequired {
            builder.diagnostic(
                ...
                Some(span),
                format!(
                    "Province {id} has a nearby inline comment; the comment will be preserved."
                ),
                "Review the preview to confirm the comment remains meaningful.",
            );
        }
        builder.delete("provinces", span, format!("Remove Province {id}"), safety);
        builder.changes.push(format!("remove Province {id}"));
    }
    let added = working
        .provinces
        .difference(&baseline.provinces)
        .copied()
        .collect::<Vec<_>>();
    if !added.is_empty() {
        let values = added
        ...
        builder
            .changes
            .extend(added.into_iter().map(|id| format!("add Province {id}")));
    }
}
...
fn plan_history(
    ...
    let Some(container) = containers.first().copied() else {
        ...
        builder
            .changes
            .push("history: create missing block".to_owned());
        return;
    };
...
fn plan_victory_points(
    ...
    let Some(container) = containers.first().copied() else {
        ...
        builder.changes.extend(
            working_map
                .iter()
                .map(|(province, value)| format!("add VP {province} = {value}")),
        );
        return;
    };
    let Some(block) = as_block(container) else {
        ...
        .collect::<BTreeSet<_>>()
    {
        match (baseline_map.get(&province), working_map.get(&province)) {
            (Some(old), Some(new)) if old != new => {
                let Some((_, value_entry)) = bindings
                    .get(&province)
                ...
                builder
                    .changes
                    .push(format!("VP {province}: {old} -> {new}"));
            }
            (Some(old), None) => {
                let Some((province_entry, value_entry)) = bindings
                    .get(&province)
                    .and_then(|pairs| pairs.first())
                    .copied()
                else {
                ...
                let span = relocatable_span(
                    ...
                    TextSpan::new(
                        ...
                        value_entry
                            .span
                            .end()
                            .saturating_sub(province_entry.span.start),
                    ),
                );
            ...
            "Keep the original value or remove the ambiguous comment association manually.",
                ...
                builder
                    .changes
                    .push(format!("remove VP {province} = {old}"));
            }
            (None, Some(new)) => {
                ...
                let (offset, content, safety) = match relocated {
                    ...
                    None => {
                        let (offset, content) = positional_insertion(
                            builder.document,
                            block,
                            &format!("{province} {new}"),
                        );
    ...
    for province_id in province_ids {
        ...
        match (bindings.first().copied(), before, after) {
            ...
            "Apply the building edit before moving the province, or review it in a later fragment-editing phase.",
                ...
                builder
                    .changes
                    .push(format!("remove Province {province_id} buildings"));
            }
            (None, None, Some(values)) | (None, Some(_), Some(values)) => {
                ...
                let (offset, content, safety) = match relocated {
                    Some((source, entry)) => {
                        ...
                        let raw = bytes_at(source.original_bytes(), span).unwrap_or_default();
                        let (offset, content) =
                            fragment_insertion(builder.document, buildings, raw);
                        let safety = if newline(source) == newline(builder.document) {
                            PatchSafety::Safe
                        } else {
                            PatchSafety::ReviewRequired
                ...
                builder
                    .changes
                    .push(format!("add Province {province_id} buildings"));
            }
...
fn plan_creation(
    ...
    PlannedFileCreation {
        ...
        semantic_changes: vec![
            ...
            format!(
                "add {} victory point(s)",
                working.history.victory_points.len()
            ),
        ],
        diagnostics,
        safety,
...
pub fn validate_non_overlapping_patches(operations: &[TextPatchOperation]) -> Result<(), String> {
...
fn source_safety_diagnostics(document: &StateDocument, state_id: u32) -> Vec<PatchDiagnostic> {
    ...
    if !document.exact_utf8 {
        diagnostics.push(PatchDiagnostic {
            ...
            span: None,
            message: "The source required lossy UTF-8 decoding; byte spans are not safe to patch."
                .to_owned(),
            action:
                "Convert the source to valid UTF-8 without changing its semantics, then reload."
                    .to_owned(),
        });
    }
    ...
    match fs::read(&document.path) {
        Ok(current) if current.as_slice() != document.original_bytes() => {
            diagnostics.push(PatchDiagnostic {
...
fn relocated_victory_point(
    ...
    let containers = entries(history, "victory_points");
    let block = (containers.len() == 1)
        .then(|| as_block(containers[0]))
        .flatten()?;
    let scalar_entries = block
        .entries
        .iter()
        .filter(|entry| entry.key.is_none() && scalar_text(entry).is_some())
        .collect::<Vec<_>>();
    let pairs = scalar_entries
        .chunks_exact(2)
        .filter(|pair| {
            scalar_text(pair[0]).and_then(|text| text.parse::<u32>().ok()) == Some(province_id)
        })
        .collect::<Vec<_>>();
...
fn relocated_province_buildings<'a>(
    ...
    let history = non_dated_history_block(source)?;
    let containers = entries(history, "buildings");
    let buildings = (containers.len() == 1)
        .then(|| as_block(containers[0]))
        .flatten()?;
    let bindings = entries(buildings, &province_id.to_string());
    let [entry] = bindings.as_slice() else {
        return None;
    };
    Some((source, entry))
}
    ...
    block
        ...
        .filter(|entry| {
            entry
                .key
                .as_ref()
                .is_some_and(|candidate| candidate.text == key)
        })
        .collect()
}
...
fn comment_safety(document: &StateDocument, span: TextSpan) -> PatchSafety {
    let line_end = document.original_bytes()[span.end().min(document.original_bytes().len())..]
        ...
        .map_or(document.original_bytes().len(), |offset| {
            span.end() + offset
        });
...
fn indentation(document: &StateDocument, block: &PdxBlock) -> String {
    block
        ...
        .unwrap_or_else(|| {
            let parent =
                indentation_before(document.original_bytes(), block.span.start).unwrap_or_default();
            format!("{parent}    ")
        })
}
...
fn insertion_at_closing(
    ...
    let closing_on_own_line = closing_prefix
        .iter()
        .all(|byte| matches!(byte, b' ' | b'\t'));
...
fn victory_point_map(points: &[VictoryPoint]) -> BTreeMap<u32, i64> {
    points
        .iter()
        .map(|point| (point.province_id, point.value))
        .collect()
}
...
fn render_history_lines(data: &StateData) -> Vec<String> {
    ...
    lines.extend(
        data.history
            .cores
            .iter()
            .map(|tag| format!("add_core_of={tag}")),
    );
    lines.extend(
        data.history
            .claims
            .iter()
            .map(|tag| format!("add_claim_by={tag}")),
    );
...
fn summarize(
    ...
    PatchPlanSummary {
        ...
        safe_files: safeties
            .iter()
            .filter(|&&safety| safety == PatchSafety::Safe)
            .count(),
        review_required_files: safeties
            .iter()
            .filter(|&&safety| safety == PatchSafety::ReviewRequired)
            .count(),
        blocked_files: safeties
            .iter()
            .filter(|&&safety| safety == PatchSafety::Blocked)
            .count(),
    }
}
...
mod tests {
    use super::*;
    use crate::app::state::{StateHistory, extract_state, parse_text};
    use std::sync::Arc;
    ...
    fn expected_bytes_and_overlap_are_enforced() {
        ...
        assert_eq!(
            apply_operations(b"abcdef", &operations).unwrap(),
            b"abXYef!"
        );
    ...
    fn ambiguous_or_changed_sources_are_blocked_without_operations() {
        let text = "state={ id=1 provinces={ 1 } resources={ oil=4 oil=4 } history={ owner=TAG owner=ALT } }";
