...
pub use indexes::{index_state_documents, StateIndexes};
pub use lasso::{
...
pub use view::{
  ...
  generate_state_view, generate_state_view_for, generate_state_view_region_for,
  select_state_at, select_state_at_for, selection_overlay, selection_overlay_for, state_color
};

use std::collections::{BTreeMap, BTreeSet, HashMap};
use std::time::Instant;

use crate::app::state::{load_state_documents, StateDocument, StateLoadBatch};
...
impl Hoi4Project {
  ...
  pub fn load_states(
    ...
    let indexes = index_state_documents(
      &batch.documents,
      valid_province_ids,
      land_province_ids
    );
...
impl StateLoadSummary {
  fn from_load(
    ...
    let count = |kind| diagnostics.iter()
    ...
    Self {
      ...
      invalid_states: batch.documents.len().saturating_sub(indexes.indexed_state_count),
...
mod tests {
  ...
  fn loads_documents_indexes_and_summary_without_writing_state_files() {
    ...
    let original = "state={id=1 name=Test state_category=city provinces={1 2} history={owner=TAG}}";
    ...
    assert!(project.load_summary_message().contains("- indexed states: 1"));
