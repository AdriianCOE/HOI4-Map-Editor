...
type DiagnosticIdentity<'a> = (
  DiagnosticSeverity, DiagnosticDomain, &'a str, &'a Option<PathBuf>, Option<u32>, Option<u32>,
  Option<(usize, usize)>
);
...
impl ProjectDiagnostic {
  ...
  pub fn summary(diagnostics: &[Self]) -> DiagnosticSummary {
    let mut summary = DiagnosticSummary { total: diagnostics.len(), ..Default::default() };
  ...
  fn compare(left: &Self, right: &Self) -> Ordering {
    (
      left.severity.rank(), left.domain, &left.path, left.state_id, left.province_id,
      &left.code, left.span.map(|span| (span.start, span.len))
    ).cmp(&(
      right.severity.rank(), right.domain, &right.path, right.state_id, right.province_id,
      &right.code, right.span.map(|span| (span.start, span.len))
    ))
  }

  fn identity(&self) -> DiagnosticIdentity<'_> {
    (
      self.severity, self.domain, &self.code, &self.path, self.state_id, self.province_id,
...
mod tests {
  ...
  fn diagnostics_are_sorted_deduplicated_and_summarized_by_stable_identity() {
    ...
    let duplicate = ProjectDiagnostic { message: "translated warning".to_owned(), ..warning.clone() };
    ...
    assert_eq!(ProjectDiagnostic::summary(&diagnostics), DiagnosticSummary {
      total: 3,
      information: 1,
      warnings: 1,
      errors: 1,
