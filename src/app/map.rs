...
impl Bundle {
    ...
    pub fn image_buffer_mapgen_land(&self) -> Option<RgbImage> {
        self.map
            .gen_image_buffer(|which| self.map.get_province(which).kind.color_mapgen())
    }
    ...
    pub fn texture_buffer_selective_color(&self, extents: Extents) -> RgbaImage {
        self.map
            .gen_texture_buffer_selective(extents, |which| which)
    }

    pub fn texture_buffer_kind(&self) -> RgbaImage {
        self.map
            .gen_texture_buffer(|which| self.map.get_province(which).kind.color())
    }
    ...
    pub fn texture_buffer_terrain(&self) -> RgbaImage {
        self.map.gen_texture_buffer(|which| {
            ...
            match self.config.terrain_color(terrain) {
                None => panic!(
                    "unknown terrain type, color not found in config: {}",
                    terrain
                ),
                Some(color) => color,
            }
...
impl std::fmt::Debug for MapBase {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("MapBase")
            ...
            .field(
                "connection_data_map",
                &format_args!("{:p}", self.connection_data_map),
            )
            .finish()
    }
}
...
impl Map {
    ...
    where
        F: Fn(Color) -> Color + Send + Sync,
    {
        let [width, height] = self.dimensions();
        RgbaImage::from_par_fn(width, height, |x, y| Rgba(p4(f(self.get_color_at([x, y])))))
    }
    ...
    where
        F: Fn(Color) -> Option<Color> + Send + Sync,
    {
        let [width, height] = self.dimensions();
    ...
    fn put_pixel_raw(&mut self, pos: Vector2<u32>, color: Color) -> Option<Color> {
        ...
        let entry = Arc::make_mut(&mut self.base.province_data_map)
            .entry(color)
            .or_default();
    ...
    fn put_selective_raw(&mut self, buffer: &RgbImage, offset: Vector2<u32>) {
        ...
        Arc::make_mut(&mut self.base.color_buffer)
            .copy_from(buffer, offset[0], offset[1])
            .expect("error");
    }
    ...
    pub fn calculate_coastal_provinces(&self) -> AHashMap<Color, Option<bool>> {
        let mut coastal_provinces = self
            .base
            .province_data_map
            .keys()
    ...
    fn iter_pixels_adjacent(
        &self,
        pos: Vector2<u32>,
    ) -> impl Iterator<Item = Vector2<u32>> + 'static {
        let [width, height] = self.dimensions();
        CARDINAL.into_iter().filter_map(move |diff| {
    ...
    fn iter_pixel_pairs(&self) -> impl Iterator<Item = (Vector2<u32>, Vector2<u32>)> + 'static {
        let [width, height] = self.dimensions();
        XYIter::new(0..width, 0..height).flat_map(move |pos| {
    ...
    fn iter_pixel_pairs_extents(
        &self,
        mut extents: Extents,
    ) -> impl Iterator<Item = (Vector2<u32>, Vector2<u32>)> + 'static {
        let [width, height] = self.dimensions();
        ...
        XYIter::from_extents(extents).flat_map(move |pos| {
    ...
    fn rekey_province_raw(&mut self, which: Color, color: Color) {
        ...
        let province_data = province_data_map
            .remove(&which)
    ...
    fn rekey_connections_raw(&mut self, which: Color, color: Color) {
        if !self.base.connection_data_map.is_empty() {
            ...
            for (&rel, connection_data) in self.base.connection_data_map.iter() {
                ...
                new_connection_data_map
                    .insert(rel.replace(&which, &color), connection_data.clone());
            }
    ...
    pub fn recolor_province(&mut self, which: Color, color: Color) -> Extents {
        assert_ne!(
            which, color,
            "Attempted to recolor a province when it is already the desired color"
        );
    ...
    pub fn merge_province_into(&mut self, which: Color, target: Color) -> Extents {
        ...
        let extents = pixels
            .iter()
            .copied()
            .fold(None, |out: Option<Extents>, pos| {
                Some(out.map_or_else(
                    || Extents::new_point(pos),
                    |current| current.join_point(pos),
                ))
            })
    ...
    pub fn flood_fill_province(&mut self, pos: Vector2<u32>, color: Color) -> Extents {
        ...
        assert_ne!(
            which, color,
            "Attempted to flood-fill a province when it is already the desired color"
        );
    ...
    pub fn get_connection(&self, rel: UOrd<Color>) -> &ConnectionData {
        self.base
            .connection_data_map
            .get(&rel)
            .expect("connection not found with rel")
    }
    ...
    pub fn get_connection_nearest_within(
        &self,
        pos: Vector2<f64>,
        range: f64,
    ) -> Option<&ConnectionData> {
        self.get_rel_nearest(pos)
            .and_then(|(rel, dist)| (dist < range).then(|| self.get_connection(rel)))
    }

    pub fn get_connection_nearest(&self, pos: Vector2<f64>) -> Option<&ConnectionData> {
        self.get_rel_nearest(pos)
            .map(|(rel, _)| self.get_connection(rel))
    }

    pub fn get_rel_nearest(&self, pos: Vector2<f64>) -> Option<(UOrd<Color>, f64)> {
        use geo::algorithm::closest_point::ClosestPoint;
        use geo::algorithm::euclidean_distance::EuclideanDistance;
        use geo::{Closest, Line, Point};
...
impl Extents {
    ...
    pub fn contains(self, point: Vector2<u32>) -> bool {
        self.upper[0] >= point[0]
            && self.lower[0] <= point[0]
            && self.upper[1] >= point[1]
